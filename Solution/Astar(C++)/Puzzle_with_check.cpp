...
void Solver::checkPath(short(*board)[5]) {
	char ipt[300];
	...
	for (auto move : ipt) {
		if (move == 'U') solution_Point.push_back(UP);
		else if (move == 'D') solution_Point.push_back(DOWN);
		else if (move == 'L') solution_Point.push_back(LEFT);
		else if (move == 'R') solution_Point.push_back(RIGHT);
		else continue;
	}
